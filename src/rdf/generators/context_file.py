...
Position
--------
Creates and updates .context.md files with human-preserved sections and
...
@dataclass
class ContextFileSections:
    """
    Parsed sections of a .context.md file.
...
class ContextFileGenerator:
    """
    Generate and update .context.md files.
    ...
    ----------
    target_dir : Path
        Directory to generate .context.md for. Human sections within this
        directory's .context.md are never overwritten. A backup is created
        before any write, and dry-run mode shows preview without changes.

    Examples
    --------
    >>> generator = ContextFileGenerator(Path("src/processors/"))
    ...
    def __init__(self, target_dir: Path) -> None:
        ...
        self.context_md_path = target_dir / ".context.md"

    def parse_existing(self) -> ContextFileSections | None:
        """
        Parse existing .context.md if present.

        Returns
        -------
        ContextFileSections | None
            Parsed sections, or None if file doesn't exist.
        """
        if not self.context_md_path.exists():
            return None

        content = self.context_md_path.read_text()

        # Find AUTO-GENERATED marker
        auto_match = re.search(AUTO_MARKER, content)
        if not auto_match:
            # No auto section - entire file is human-authored
            return ContextFileSections(
                preamble=content,
                human_sections="",
                auto_section="",
            )
...
"""
        ...
        if dry_run:
            return f"Would write to {self.context_md_path}:\n\n{full_content}"

        # Create backup
        if self.context_md_path.exists():
            backup_path = self.context_md_path.with_suffix(".md.bak")
            backup_path.write_text(self.context_md_path.read_text())

        # Write
        self.context_md_path.write_text(full_content)
        return full_content
