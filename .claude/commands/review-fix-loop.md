---
description: Run adversarial review-fix loop on the local diff (pre-PR) using sub-agents
---

# Review-Fix Loop

You are a **coordinator** orchestrating an adversarial review-fix loop on the LOCAL diff (against a base branch). You do NOT review or fix code yourself — you spawn sub-agents for each role and manage the loop.

**This command runs BEFORE `gh pr create`** (per T0.HARN-013). It operates on `git diff <base>...HEAD` — no PR required. The PR is opened after the loop completes, so reviewers see the final, polished diff rather than the audit trail of getting there.
...
**Usage:**
- `/review-fix-loop` — auto-detect base branch (upstream → origin/dev → origin/main → origin/master)
- `/review-fix-loop --base origin/main`
- `/review-fix-loop --num-rounds 2`
- `/review-fix-loop --base origin/dev --num-rounds 3`
...
### Detected Base Branch (upstream → origin/dev → origin/main → origin/master)
...
1. **Base branch** — `--base <ref>`. If omitted, use the "Detected Base Branch" pre-computed value.
2. **Num rounds** — `--num-rounds N` (default: 1 when auto-chained from `/epic-oneshot` / `/commit-push-pr` / `/implement`; default 3 when invoked manually).

If no base branch can be determined (empty pre-computed value, no `--base` flag):
```
No base branch detected. Specify with --base <ref>.
Usage: /review-fix-loop [--base <ref>] [--num-rounds N]
```
...
Set:
- `round = 1`
- `max_rounds` = parsed value or 1 (auto-chain) / 3 (manual invoke)
- `approved = false`
- `base = ` resolved base branch (e.g., `origin/dev`)
- `branch = ` current branch (from pre-computed context)
- `branch_slug = ` sanitized branch name for review-file naming
- `date` = today's date from pre-computed context
- `reviewer_username` = from pre-computed context

Report to the user:
```
Starting adversarial review-fix loop against [BASE]
Branch: [BRANCH]
Max rounds: [N]
```
...
```
You are an adversarial code reviewer. Review this diff with fresh eyes — you have no context about how the code was written or what trade-offs were made. This is a PRE-PR review: there is no PR yet, only a local diff.

## Your Task

Review the local diff on branch `[BRANCH]` against `[BASE]` and write a structured review file.

## Step 1: Gather Context

Run these commands to get the diff information:

1. Get the LOCAL diff against the base branch:
   `git diff [BASE]...HEAD | head -2000`
   If the diff is truncated, use file-specific diffs for files you need to review in full:
   `git diff [BASE]...HEAD -- path/to/specific/file`

2. Get diff stats (file list and change counts):
   `git diff --stat [BASE]...HEAD`

3. Get commit history on this branch:
   `git log --oneline [BASE]...HEAD`

4. If a plan file is at `[PLAN_FILE]` (resolved from the pre-computed context), read it for intent.
...
Create the review file at: `thoughts/reviews/[DATE]-[BRANCH_SLUG]-review-[ROUND].md`

Where:
- DATE = [DATE]
- BRANCH_SLUG = sanitized current branch name (lowercase; `/` → `-`; e.g. `feat-T0-HARN-013-...`)
- ROUND = [ROUND]
...
```
You are a code fixer. Your job is to address review feedback on branch [BRANCH] systematically.
...
Create a response file at: `thoughts/reviews/[DATE]-[BRANCH_SLUG]-response-[ROUND].md`
